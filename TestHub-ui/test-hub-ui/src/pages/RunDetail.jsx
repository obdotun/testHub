...
export default function RunDetail() {
  ...
  return (
    <Box sx={{ p: 3, overflow: 'auto', flex: 1 }}>
      ...
      {isActive && (
        <LinearProgress color="warning" sx={{ mb: 2, borderRadius: 1 }} />
      )}
        ...
        <Paper sx={{ p: 2, mb: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            ...
            <Typography variant="caption" fontWeight={700}
              sx={{ color: passRate === 100 ? 'success.main' : passRate > 50 ? 'warning.main' : 'error.main' }}>
              {passRate}%
            </Typography>
          </Box>
          <LinearProgress
      ...
      <Paper sx={{ mb: 2 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)}
          ...
          <Tab label={`📟 Logs temps réel (${logs.length})`} />
